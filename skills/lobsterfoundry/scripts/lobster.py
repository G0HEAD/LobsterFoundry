...
# Default server
DEFAULT_SERVER = os.environ.get('LOBSTER_BASE_URL') or os.environ.get('LOBSTER_SERVER', 'http://localhost:5173')
...
def load_public_key_from_file(path):
    ...
    if isinstance(parsed, dict):
        return parsed.get('public_key') or parsed.get('publicKey')
...
def cmd_register(args):
    ...
    if args.public_key_file:
        key_path = Path(args.public_key_file)
        if not key_path.exists():
            print(f"❌ Public key file not found: {args.public_key_file}")
            sys.exit(1)
        public_key = load_public_key_from_file(args.public_key_file)
        if not public_key:
            print(f"❌ Public key file is empty: {args.public_key_file}")
            sys.exit(1)

    if not public_key and args.public_key:
...
def main():
    ...
    p_register.add_argument('--public-key', help='Public key string')
    p_register.add_argument('--public-key-file', help='Path to JSON or raw public key')
